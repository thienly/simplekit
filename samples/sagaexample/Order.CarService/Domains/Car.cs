using System;
using System.Collections.Generic;
using System.Linq;
using SimpleKit.Domain.Entities;

namespace Order.CarService.Domains
{
    public class DomainException : Exception
    {
        public DomainException(string message) : base(message)
        {
        }
    }

    public class Car : AggregateRootBase
    {
        public string Name { get; set; }
        public CarType CarType { get; set; }
        public decimal Price { get; set; }

        public List<CarReservation> CarReservations { get; set; } = new List<CarReservation>();

        public CarReservation Booking(DateTime startDate, DateTime endDate)
        {
            if (startDate > endDate)
                throw new DomainException("The Start Date can not greeter then End Date");
            if (CarReservations.Any(d =>
                d.EndDate <= startDate || (d.StartDate <= startDate && d.EndDate >= endDate && d.IsBooked)))
                throw new DomainException($"This care has been booked from {startDate} to {endDate}");
            var reservation = new CarReservation()
            {
                CarId = this.Id,
                StartDate = startDate,
                EndDate = endDate
            };
            CarReservations.Add(reservation);
            return reservation;
        }

        public void Cancel(Guid bookingId)
        {
            var carReservation = CarReservations.Single(x => x.Id == bookingId);
            carReservation.IsBooked = false;
        }

        public decimal GetPricing(DateTime startDate, DateTime endDate)
        {
            return 100;
        }
    }
}